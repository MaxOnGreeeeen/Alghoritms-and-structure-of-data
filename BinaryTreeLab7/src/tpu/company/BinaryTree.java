...
public class BinaryTree {
        private Node root;
        public BinaryTree(){
            root = null; //now it is empty
        }

        public Node find(Time time, int key){//поиск узла с заданным ключем
                time.start();
            Node currentEl = root;
            while(currentEl.iData != key){
                    if ( key < currentEl.iData)
                            currentEl = currentEl.leftChild;//is the element searched left child
                    else
                            currentEl = currentEl.rightChild;//is the element searched right child
                    if (currentEl == null){
                            System.out.println("Element was not found");
                            return null;//the element is not found
                    }
            }time.finish();
            System.out.println("Element " + key + " was found in its position.");
            return currentEl;
        }
        ...
        public void insert(int id) {
                ...
                newNode.iData = id; // Insert the data
                if(root==null) // Root node is empty
                        root = newNode;
